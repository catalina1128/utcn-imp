...
void Codegen::LowerIfStmt(const Scope &scope, const IfStmt &ifStmt)
{
  auto entry = MakeLabel();
  auto exit = MakeLabel();
  auto elsestmt = MakeLabel();
  
  EmitLabel(entry);
  LowerExpr(scope, ifStmt.GetCond());
  EmitJumpFalse(elsestmt);
  EmitLabel(elsestmt);
...
void Codegen::LowerIntExpr(const Scope &scope, const IntExpr &intValue)
{
  depth_ += 1;
  Emit<Opcode>(Opcode::PUSH_INT);
  Emit<uint64_t>(intValue.GetValue());
}
// -----------------------------------------------------------------------------
